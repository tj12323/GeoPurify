...
class XDecoder(nn.Module):
    ...
    def forward(self, x, mask_features, mask=None, target_queries=None, target_vlp=None, task='seg', extra={}):
        ...
        predictions_embed.append(results["outputs_embed"])
        ...
        if task == 'vlp':
            ...
            out = {
                ...
                'mask_embed':    predictions_embed[-1],
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size, layer_id=-1, task='seg'):
        ...
        results = {
            ...
            "outputs_embed": class_embed if self.task_switch['mask'] else None, # 使用为分类而生的 class_embed
    ...
    @torch.jit.unused
    def _set_aux_loss(self, outputs_class, outputs_seg_masks, outputs_boxes, outputs_captions,outputs_embed):
        if self.mask_classification:
            return [
                ...
                for a, b, e, c, d in zip(
                    outputs_class[:-1],
                    outputs_seg_masks[:-1],
                    outputs_embed[:-1],       # 从这里取
                    outputs_boxes[:-1],
                    outputs_captions[:-1],
                )
            ]
            ...
            return [
                ...
                for b, e in zip(outputs_seg_masks[:-1], outputs_embed[:-1])
            ]
