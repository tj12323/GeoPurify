...
class GeneralizedXdecoder(nn.Module):
    ...
    @classmethod
    def from_config(cls, cfg,backbone=None):
    ...
    def forward_seg_all(self, batched_inputs):
        images = [x["image"].squeeze(0).to(self.device) for x in batched_inputs]
        images = [(x - self.pixel_mean) / self.pixel_std for x in images]
        images = ImageList.from_tensors(images, self.size_divisibility)
        ...
        mask_features, raw_outputs = self.sem_seg_head(features, target_queries=queries_grounding)
        # Process the main outputs
        final_outputs = self._process_and_slice_outputs(raw_outputs)
        # Process the auxiliary outputs
        final_aux_outputs = self._slice_aux_output(raw_outputs['aux_outputs'])

        # Include logit_scale in aux_outputs
        logit_scale = self.sem_seg_head.predictor.lang_encoder.logit_scale.exp()
        for aux_output in final_aux_outputs:
            aux_output['logit_scale'] = logit_scale  # Add logit_scale to each auxiliary output
        
        # Update the final outputs with aux_outputs
        final_outputs['aux_outputs'] = final_aux_outputs
        all_embeds = getattr(self.sem_seg_head.predictor.lang_encoder, '{}_text_embeddings'.format('default'))
        text_embed = all_embeds[:-1, :]
        null_embed = all_embeds[-1:, :]
        ...
        final_outputs.update(extra_info)

        return features, final_outputs

    def _slice_aux_output(self, aux_list: List[dict]) -> List[dict]:
        sliced = []
        for layer_dict in aux_list:
            d = {}
            # 注意：queries-1 是因为你把最后一个 cls_token 也放到 pred_logits 里了
            d['pred_logits'] = layer_dict['pred_logits'][:, :self.num_queries-1]
            d['pred_masks']  = layer_dict['pred_masks'][:,  :self.num_queries-1]
            d['mask_embed']  = layer_dict['mask_embed'][:,  :self.num_queries-1]
            if 'pred_captions' in layer_dict:
                d['pred_captions'] = layer_dict['pred_captions'][:, :self.num_queries-1]
            # grounding 部分
            if self.task_switch['grounding']:
                d['pred_gmasks'] = layer_dict['pred_masks'][:,  self.num_queries:2*self.num_queries-1]
                d['pred_gtexts'] = layer_dict['pred_captions'][:, self.num_queries:2*self.num_queries-1]
            sliced.append(d)
        return sliced

    def _process_and_slice_outputs(self, predictions: dict) -> dict:
        """
        Processes a single dictionary of predictions, slicing tensors
        for different tasks (e.g., standard vs. grounding).
        """
        processed_preds = {}
        for key, value in predictions.items():
            if key == 'pred_logits':
                processed_preds[key] = value[:, :self.num_queries-1]
            
            elif key == 'pred_masks':
                processed_preds[key] = value[:, :self.num_queries-1]
                if self.task_switch['grounding']:
                    # Note: Slicing assumes a specific concatenation order from the head
                    processed_preds['pred_gmasks'] = value[:, self.num_queries:2*self.num_queries-1]
                ...
                processed_preds[key] = value[:, :self.num_queries-1]
                if self.task_switch['grounding']:
                    processed_preds['pred_gtexts'] = value[:, self.num_queries:2*self.num_queries-1]
            
            # We don't process 'aux_outputs' here, it will be handled by the main forward pass
            elif key != 'aux_outputs':
                processed_preds[key] = value # Copy over any other keys

        return processed_preds
    
    def forward_seg_xmask3d(self, features):
        # 1. Get all raw predictions from the head.
        extra = {}
        mask_features, raw_outputs = self.sem_seg_head(features, extra=extra)

        # 2. Process the auxiliary outputs in a loop.
        final_aux_outputs = []
        if 'aux_outputs' in raw_outputs:
            for raw_aux_pred in raw_outputs['aux_outputs']:
                # 2a. First, slice the raw auxiliary prediction.
                processed_aux = self._process_and_slice_outputs(raw_aux_pred)

                # 2b. Now, run the post-processor on this sliced auxiliary prediction
                #      to generate the extra keys ('mask_embed', etc.).
                aux_post_results = self.odise_post_processor(
                    pred_masks=processed_aux['pred_masks'],
                    mask_features=mask_features
                )

                # 2c. Combine the sliced results with the post-processor results.
                processed_aux.update(aux_post_results)
                
                # 2d. Append the complete dictionary to our list of auxiliary outputs.
                final_aux_outputs.append(processed_aux)

        # 3. Process the final layer's predictions using the same full sequence.
        # 3a. Slice the raw final prediction.
        final_outputs = self._process_and_slice_outputs(raw_outputs)
        # 3b. Run the post-processor on the final prediction.
        final_post_results = self.odise_post_processor(
            pred_masks=final_outputs['pred_masks'],
            mask_features=mask_features
        )
        
        # 3c. Combine the results for the final output.
        final_outputs.update(final_post_results)

        # 4. Assemble the complete results dictionary.
        final_outputs['aux_outputs'] = final_aux_outputs

        # 5. Add any other metadata that is only for the top-level output.
        extra_info = {
    ...
    def evaluate(self, batched_inputs):
        ...
        targets = targets_grounding = queries_grounding = None
        features = self.backbone(images.tensor)
        _,outputs = self.sem_seg_head(features, target_queries=queries_grounding)
